...
export async function POST(req: Request) {
  ...
  const candidates = matchPlaces(tags).slice(0, 12);
  ...
  if (geminiReady() && candidates.length) {
    try {
      const list = candidates
        .map((p) => `${p.id}: ${p.name} (${p.kind}, ${p.area}) — ${p.note}`)
        .join("\n");
...
Return STRICT JSON: {"recommendations":[{"id":"<place id>","reason":"<sentence>"}]}`,
  ...
  if (!recos.length) {
    recos = candidates.slice(0, 6).map((p) => ({
      id: p.id,
      reason: p.note,
    }));
  }
  ...
  const places = recos
    .map((r) => ({ place: byId.get(r.id) as Place, reason: r.reason }))
    .filter((x) => x.place);
